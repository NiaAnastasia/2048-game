...
document.addEventListener("touchend", (e)=>{
    ...
    if (Math.abs(dx) > Math.abs(dy)) dx > 0 ? game.moveRight() : game.moveLeft();
    else dy > 0 ? game.moveDown() : game.moveUp();
    render();
});
...
function render() {
    ...
    if (currentStatus === "win") winMsg.classList.remove("hidden");
    if (currentStatus === "lose") loseMsg.classList.remove("hidden");
}
...
class Game {
   ...
   */ constructor(initialState){
    ...
    moveRight() {
        this._applyMove(this.board.map((row)=>this.slide(row.toReversed()).toReversed()));
    }
    moveUp() {
    ...
    moveDown() {
        let tempBoard = this.transpose(this.board);
        tempBoard = tempBoard.map((row)=>this.slide(row.toReversed()).toReversed());
        this._applyMove(this.transpose(tempBoard));
    }
    transpose(matrix) {
    ...
    canMove() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (c < 3 && current === this.board[r][c + 1]) return true;
            if (r < 3 && current === this.board[r + 1][c]) return true;
        }
        return false;
    }
    slide(row) {
        const list = row.filter((x)=>x !== 0);
        for(let i = 0; i < list.length - 1; i++)if (list[i] === list[i + 1]) {
            list[i] *= 2;
            this.score += list[i];
            list.splice(i + 1, 1);
        }
